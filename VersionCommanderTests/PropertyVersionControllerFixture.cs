...
namespace VersionCommander.UnitTests
{
    ...
    public class PropertyVersionControllerFixture
    {
        ...
        private const string ExistingControlNodeArgumentName = "existingControlNode";
        private const string BaseObjectArgumentName = "existingObject";
        ...
        public void SetupTestingAssists()
        {
            ...
            //see calledWithBaseObjectAndChangeSet(), used in the test when_getting_version_without_modifications_past_specific_version()
            var parameterNames = MethodInfoExtensions.GetMethodInfo<IProxyFactory>(x => x.CreateVersioning<object>(null, null, null))
                                                     .GetParameters()
                                                     .Select(param => param.Name);

            parameterNames.Should().Contain(new[] { ExistingControlNodeArgumentName, BaseObjectArgumentName });
        }
        ...
        {
            var correct = args.Get<IVersionControlNode>(ExistingControlNodeArgumentName).Mutations.Single().Equals(versionDelta);
